...
@section('content')
    ...
    @foreach(PostRepository::fetchLatestPosts()['data'] as $key => $row)
        <article class="amp-wp-article">
            <div class="amp-wp-article-categories">
                <div class="post-categories-container">
                    <div class="post-categories">
                        <a href="{{url('/amp/category/'.$row->category_slug)}}"
                            title="{{ $row->category }}">
                            {{ $row->category }}
                        </a>
                    </div>
            ...
            <header class="amp-wp-article-header">
                <a href="{{url('/amp/post/'.$row->slug_url)}}">
            ...
            <figure class="amp-wp-article-featured-image wp-caption">
                <a href="{{url('/amp/post/'.$row->slug_url)}}">
                    <amp-img width="1024" height="768" src="{{ $row->image }}" 
                        ...
                        <noscript>
                            <img width="1024" height="768" src="{{ $row->image }}" 
