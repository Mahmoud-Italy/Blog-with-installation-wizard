        ...
        <main class="u-main">
            ...
            <div class="u-content">
                <div class="u-body min-h-700">
                    <h1 class="h2 mb-2 text-capitalize">Roles
                    ...
                    <div class="card">
                        <header class="card-header">
                            ...
                            <router-link class="pg-hd" 
                                :to="{ name: 'roles' }"
                                :class="(this.status == '') ? 'active' : '' ">All
                            </router-link> 
                            <span class="pg-hd no-decoration f14"> ({{count_all}}) </span> &nbsp;|&nbsp;

                            <router-link class="pg-hd" 
                                :to="{ name: 'status-role', params:{status: 'active'} }"
                                :class="(this.status == 'active') ? 'active' : '' ">Active
                            </router-link> 
                            <span class="pg-hd no-decoration f14"> ({{count_active}}) </span> &nbsp;|&nbsp;

                            <router-link class="pg-hd"
                                :to="{ name: 'status-role', params:{status: 'inactive'} }"
                                :class="(this.status == 'inactive') ? 'active' : '' ">Inactive
                            </router-link> 
                            <span class="pg-hd no-decoration f14"> ({{count_inactive}}) </span> &nbsp;|&nbsp;

                            <router-link class="pg-hd"
                                :to="{ name: 'status-role', params:{status: 'trash'} }"
                                :class="(this.status == 'trash') ? 'active' : '' ">Trash
                            </router-link> 
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="form-group">
                            <button class="btn btn-primary" :disabled="btnLoading">
                                ...
                                <span v-if="!btnLoading"> {{btn_status}} Role</span>
                            </button>
                        </div>
                        <!-- End Button -->
    ...
    export default {
        ...
        methods: {
            ...
            fetchData(page_url, loading=false) {
                ...
                page_url = page_url || '/api/v1/dashboard/roles';
            ...
            async fetchExport(){
                const res = 
                await axios.get('/api/v1/dashboard/roles/export/file?ids='+this.selected+'&api_key='+window.api_key);
            ...
            addNew(){
                ...
                let permission = 'admin.roles.create';
                if(this.edit) {
                ...
                // In case Update
                let req = '/api/v1/dashboard/roles';
                if(this.edit) {
                    formData.append('_method', 'PUT');
                    req = '/api/v1/dashboard/roles/'+this.row.id;
                }
        ...
        /** Bulk Actions **/
            ...
            activeOrInactive(id){
                ...
                formData.append('api_key', window.api_key);
                formData.append('_method','PUT');
                axios.post('/api/v1/dashboard/roles/status/'+id, formData, config)
                .then(res => {
            ...
            moveToTrash(id){
                ...
                axios.post('/api/v1/dashboard/roles/trash/'+id, formData, config)
                .then(res => {
                    if(res.data.statusCode == 200) {
                        izitoast.success({
                            icon: 'ti-check',
            ...
            restoreFromTrash(id){
                ...
                axios.post('/api/v1/dashboard/roles/restore/'+id, formData, config)
                .then(res => {
                    if(res.data.statusCode == 200) {
                        izitoast.success({
                            icon: 'ti-check',
            ...
            deletePermanently(id) {
                if(confirm('Are You Sure?')) {
                    ...
                    formData.append('api_key', window.api_key);
                    formData.append('_method', 'DELETE');
                    axios.post('/api/v1/dashboard/roles/'+id, formData, config)
                    .then(res => {
                        if(res.data.statusCode == 200) {
                            izitoast.success({
                                icon: 'ti-check',
